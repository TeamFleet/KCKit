...
module.exports = (
    ...
    if (
        typeof equipmentStars === 'number' ||
        typeof equipmentStars === 'string'
    )
        equipmentStars = [equipmentStars];
    if (
        typeof equipmentRanks === 'number' ||
        typeof equipmentRanks === 'string'
    )
    ...
    equipments = [
        ...Array(Math.max(maxSlotsPlusExtra, ship.slot.length + 1)),
    ].map((_, index) => {
    ...
    return {
        ship,
        equipments,
        equipmentStars: equipments.map((_, index) =>
            typeof equipments[index] === 'object'
                ? Math.min(10, parseInt(equipmentStars[index]) || 0)
                : undefined
        ),
        equipmentRanks: equipments.map((_, index) =>
            typeof equipments[index] === 'object'
