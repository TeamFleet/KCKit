...
module.exports = (ship, equipments) => {
    ...
    if (Array.isArray(OASWs)) {
        equipments = equipments.filter((equipment, index) =>
            ship.canEquipThis(equipment, index)
        );
        ...
        OASWs.filter(
            ...
            if (OASW.equipments) {
                if (!checkEquipments(equipments, OASW.equipments)) return;
            }
            let value = equipments.reduce((requiredASW, equipment) => {
                if (equipment) return requiredASW - equipment.stat.asw;
                return requiredASW;
            }, OASW.shipWithEquipments.hasStat.asw);
        ...
        if (resultLevel > maxShipLv) return false;
        if (resultLevel < ship._minLv) return ship._minLv;
        return resultLevel;
    } else {
        return false;
    }
