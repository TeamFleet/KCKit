...
describe('Checking functions/utilities', () => {
    describe('Checking for ship...', () => {
        ...
        it(`should condition isName be working`, function() {
            ...
            expect(check.ship(1, { isName: ['mutsuki', 'satsuki'] })).toBe(
                true
            );
            ...
            expect(
                check.ship(db.ships[1], { isName: ['mutsuki', 'satsuki'] })
            ).toBe(true);
            expect(
                check.ship(db.ships[1], { isName: ['satsuki', 'yayoi'] })
            ).toBe(false);
        });
        it(`should condition isNotName be working`, function() {
            ...
            expect(check.ship(1, { isNotName: ['mutsuki', 'satsuki'] })).toBe(
                false
            );
            expect(check.ship(1, { isNotName: ['satsuki', 'yayoi'] })).toBe(
                true
            );
            expect(check.ship(db.ships[1], { isNotName: 'mutsuki' })).toBe(
                false
            );
            expect(check.ship(db.ships[1], { isNotName: '睦月' })).toBe(false);
            expect(check.ship(db.ships[1], { isNotName: 'むつき' })).toBe(
                false
            );
            expect(check.ship(db.ships[1], { isNotName: 'satsuki' })).toBe(
                true
            );
            expect(
                check.ship(db.ships[1], { isNotName: ['mutsuki', 'satsuki'] })
            ).toBe(false);
            expect(
                check.ship(db.ships[1], { isNotName: ['satsuki', 'yayoi'] })
            ).toBe(true);
        });
        ...
        it(`should condition isNotClass be working`, function() {
            ...
            expect(check.ship(db.ships[1], { isNotClass: [12, 1] })).toBe(
                false
            );
    ...
    describe('Checking for equipment...', () => {
        ...
        it(`should condition isNotID be working`, function() {
            ...
            expect(check.equipment(db.items[1], { isNotID: [1, 2] })).toBe(
                false
            );
            expect(check.equipment(db.items[1], { isNotID: [2, 3] })).toBe(
                true
            );
        });
        it(`should condition isName be working`, function() {
            ...
            expect(
                check.equipment(1, { isName: ['11cm単装砲', '12cm単装砲'] })
            ).toBe(true);
            expect(
                check.equipment(1, { isName: ['12cm连装砲', '13cm连装砲'] })
            ).toBe(false);
            expect(check.equipment(db.items[1], { isName: '12cm単装砲' })).toBe(
                true
            );
            expect(check.equipment(db.items[1], { isName: '12cm单装炮' })).toBe(
                true
            );
            expect(check.equipment(db.items[1], { isName: '12cm连装砲' })).toBe(
                false
            );
            expect(
                check.equipment(db.items[1], {
                    isName: ['11cm単装砲', '12cm単装砲']
                })
            ).toBe(true);
            expect(
                check.equipment(db.items[1], {
                    isName: ['12cm连装砲', '13cm连装砲']
                })
            ).toBe(false);
        });
        it(`should condition isNotName be working`, function() {
            ...
            expect(
                check.equipment(1, { isNotName: ['11cm単装砲', '12cm単装砲'] })
            ).toBe(false);
            expect(
                check.equipment(1, { isNotName: ['12cm连装砲', '13cm连装砲'] })
            ).toBe(true);
            expect(
                check.equipment(db.items[1], { isNotName: '12cm単装砲' })
            ).toBe(false);
            expect(
                check.equipment(db.items[1], { isNotName: '12cm单装炮' })
            ).toBe(false);
            expect(
                check.equipment(db.items[1], { isNotName: '12cm连装砲' })
            ).toBe(true);
            expect(
                check.equipment(db.items[1], {
                    isNotName: ['11cm単装砲', '12cm単装砲']
                })
            ).toBe(false);
            expect(
                check.equipment(db.items[1], {
                    isNotName: ['12cm连装砲', '13cm连装砲']
                })
            ).toBe(true);
        });
        it(`should condition isNameOf be working`, function() {
            ...
            expect(check.equipment(1, { isNameOf: ['11cm', '12cm'] })).toBe(
                true
            );
            expect(check.equipment(1, { isNameOf: ['11cm', '13cm'] })).toBe(
                false
            );
        });
        it(`should condition isType (and other similar method) be working`, function() {
            ...
            expect(check.equipment(db.items[1], { isType: [2, 3] })).toBe(
                false
            );
        ...
        it(`should condition isNotType (and other similar method) be working`, function() {
            ...
            expect(check.equipment(db.items[1], { isNotType: [1, 2] })).toBe(
                false
            );
            expect(check.equipment(db.items[1], { isNotType: [2, 3] })).toBe(
                true
            );
        });
        it(`should multiple conditions be working`, function() {
    ...
    describe('Checking for equipment list...', () => {
        it(`should condition hasID be working`, function() {
            ...
            expect(check.equipments([1, 2, 3], { hasID: [11, 12] })).toBe(
                false
            );
            expect(check.equipments([1, 2, 3], { hasID: [1, 111] })).toBe(
                false
            );
            expect(check.equipments([1, 1, 2, 3], { hasID: [1, 111] })).toBe(
                false
            );
        ...
        it(`should condition hasNameOf be working`, function() {
            expect(check.equipments([1, 83], { hasNameOf: '九三一空' })).toBe(
                true
            );
            expect(
                check.equipments([1, 83], { hasNameOf: ['12cm', '九三一空'] })
            ).toBe(true);
            expect(check.equipments([1, 83], { hasNameOf: '11cm' })).toBe(
                false
            );
            expect(
                check.equipments([1, 2, 83], {
                    hasNameOf: ['11cm', '六〇一空']
                })
            ).toBe(false);
            expect(
                check.equipments([1, 2, 83], {
                    hasNameOf: ['12cm', '11cm', '六〇一空']
                })
            ).toBe(false);
            expect(
                check.equipments([1, 1, 2, 83], {
                    hasNameOf: ['12cm', '11cm', '六〇一空']
                })
            ).toBe(false);
        });
        it(`should type conditions be working`, function() {
            ...
            expect(check.equipments([51, 51], { hasAAGun: [3, 4] })).toBe(
                false
            );
            expect(
                check.equipments([71, 131, 1], {
            ...
            expect(
                check.equipments([122, 106], {
                    hasHAMount: 2
                })
            ).toBe(false);
            expect(
                check.equipments([244, 188], {
                    hasTorpedoBomber: {
                        hasStat: {
                            asw: 7
                        }
    ...
    describe('Checking for AACI...', () => {
        describe(`Get AACI ids for ship only`, () => {
            ...
            it(`Check samples`, function() {
                // 春雨・改
                expect(check.aaci(405).map(obj => obj.id)).toEqual([
                    8,
                    7,
                    12,
                    9
                ]);
                expect(check.aaci(428).map(obj => obj.id)).toEqual([
                    10,
                    11,
                    5,
                    8,
                ...
                expect(check.aaci(136).map(obj => obj.id)).toEqual([
                    4,
                    5,
                    6,
                    8,
                ...
                /* 秋月・改 */ expect(
                    check.aaci(330).map(obj => obj.id)
                ).toEqual([1, 2, 3]);
                expect(check.aaci(141).map(obj => obj.id)).toEqual([
                    5,
                    14,
                    8,
                    7,
                ...
                expect(check.aaci(470).map(obj => obj.id)).toEqual([
                    5,
                    16,
                    8,
                    7,
                ...
                expect(check.aaci(418).map(obj => obj.id)).toEqual([
                    5,
                    8,
                    7,
                    12,
                ...
                expect(check.aaci(487).map(obj => obj.id)).toEqual([
                    19,
                    5,
                    8,
                    7,
                ...
                expect(check.aaci(488).map(obj => obj.id)).toEqual([
                    21,
                    5,
                    8,
                    7,
                ...
                expect(check.aaci(557).map(obj => obj.id)).toEqual([
                    29,
                    5,
                    8,
                    7,
        ...
        describe(`Check AACI id for ship only`, () => {
            ...
            it(`Check samples`, function() {
                ...
                expect(check.aaci(488, undefined, [21, 5, 8, 7, 12, 9])).toBe(
                    true
                );
            });
        });
        describe(`Get AACI ids for ship and equipment list`, () => {
            ...
            describe(`Check samples`, () => {
                it(`全ての水上艦：特殊機銃 + 素対空3+の対空機銃 + 対空電探`, function() {
                    // 吹雪・改二
                    expect(
                        check.aaci(426, [173, 38, 27]).map(obj => obj.id)
                    ).toEqual([12]);
                    expect(
                        check.aaci(426, [85, 38, 27]).map(obj => obj.id)
                    ).toEqual([]);
                    expect(
                        check.aaci(426, [173, 37, 27]).map(obj => obj.id)
                    ).toEqual([]);
                    expect(
                        check.aaci(426, [173, 131, 27]).map(obj => obj.id)
                    ).toEqual([12]);
                    expect(
                        check.aaci(426, [173, 38, 28]).map(obj => obj.id)
                    ).toEqual([]);
                    // 伊13・改
                    expect(
                        check.aaci(374, [173, 38, 27]).map(obj => obj.id)
                    ).toEqual([]);
                });
                it(`UIT-25 / 伊504`, function() {
                    expect(check.aaci(539, [38]).map(obj => obj.id)).toEqual([
                        23
                    ]);
                    expect(check.aaci(539, [85]).map(obj => obj.id)).toEqual([
                        23
                    ]);
                    ...
                    expect(check.aaci(530, [38]).map(obj => obj.id)).toEqual([
                        23
                    ]);
                    expect(check.aaci(530, [85]).map(obj => obj.id)).toEqual([
                        23
                    ]);
                ...
                it(`龍田・改二`, function() {
                    expect(
                        check.aaci(478, [48, 38]).map(obj => obj.id)
                    ).toEqual([24]);
                    expect(
                        check.aaci(478, [122, 38]).map(obj => obj.id)
                    ).toEqual([24]);
                    expect(
                        check.aaci(478, [48, 85]).map(obj => obj.id)
                    ).toEqual([24]);
                    expect(
                        check.aaci(478, [122, 85]).map(obj => obj.id)
                    ).toEqual([24]);
                    expect(
                        check.aaci(478, [48, 37]).map(obj => obj.id)
                    ).toEqual([]);
                    expect(
                        check.aaci(478, [122, 37]).map(obj => obj.id)
                    ).toEqual([]);
                    expect(
                        check.aaci(478, [48, 131]).map(obj => obj.id)
                    ).toEqual([]);
                    expect(
                        check.aaci(478, [122, 131]).map(obj => obj.id)
                    ).toEqual([]);
                });
                it(`伊勢・改 / 日向・改`, function() {
                    // 12cm30連装噴進砲改二 + 13号対空電探 + 三式弾
                    expect(
                        check.aaci(82, [274, 27, 35]).map(obj => obj.id)
                    ).toEqual([25, 28]);
                    // 12cm30連装噴進砲改二 + 13号対空電探
                    expect(
                        check.aaci(82, [274, 27]).map(obj => obj.id)
                    ).toEqual([28]);
                });
                it(`鬼怒・改二`, function() {
                    // Bofors 40mm四連装機関砲
                    expect(check.aaci(487, [173]).map(obj => obj.id)).toEqual([
                        20
                    ]);
                    // Bofors 40mm四連装機関砲 + 10cm連装高角砲
                    expect(
                        check.aaci(487, [173, 3]).map(obj => obj.id)
                    ).toEqual([19, 20]);
                    // Bofors 40mm四連装機関砲 + 10cm連装高角砲+高射装置
                    expect(
                        check.aaci(487, [173, 122]).map(obj => obj.id)
                    ).toEqual([20]);
                });
                it(`磯風・改二`, function() {
                    // 10cm連装高角砲
                    ...
                    // 10cm連装高角砲 + 13号対空電探
                    expect(
                        check.aaci(557, [3, 27]).map(obj => obj.id)
                    ).toEqual([29]);
                    // 10cm連装高角砲+高射装置 + 13号対空電探
                    expect(
                        check.aaci(557, [122, 27]).map(obj => obj.id)
                    ).toEqual([29, 8]);
                });
                it(`Warspite・改`, function() {
                    // 16inch Mk.I三連装砲改+FCR type284 + QF 2ポンド8連装ポンポン砲
                    expect(
                        check.aaci(364, [300, 191]).map(obj => obj.id)
                    ).toEqual([32]);
                    // 20連装7inch UP Rocket Launchers + QF 2ポンド8連装ポンポン砲
                    expect(
                        check.aaci(364, [301, 191]).map(obj => obj.id)
                    ).toEqual([32]);
                });
                it(`Nelson・改`, function() {
                    // 16inch Mk.I三連装砲改+FCR type284 + QF 2ポンド8連装ポンポン砲
                    expect(
                        check.aaci(576, [300, 191]).map(obj => obj.id)
                    ).toEqual([32]);
                    // 20連装7inch UP Rocket Launchers + QF 2ポンド8連装ポンポン砲
                    expect(
                        check.aaci(576, [301, 191]).map(obj => obj.id)
                    ).toEqual([32]);
                    ...
                    expect(
                        check.aaci(576, [301, 301]).map(obj => obj.id)
                    ).toEqual([32]);
                });
                it(`Gotland・改`, function() {
                    // 8cm高角砲 + 8cm高角砲 + 8cm高角砲
                    expect(
                        check.aaci(579, [66, 66, 66]).map(obj => obj.id)
                    ).toEqual([30]);
                    // 8cm高角砲 + 7.7mm機銃
                    expect(
                        check.aaci(579, [66, 37]).map(obj => obj.id)
                    ).toEqual([33]);
                });
                it(`Johnston / Johnston改`, function() {
                    // Johnston | 5inch単装砲 Mk.30改+GFCS Mk.37 + 5inch単装砲 Mk.30改+GFCS Mk.37
                    expect(
                        check.aaci(562, [308, 308]).map(obj => obj.id)
                    ).toEqual([34]);
                    // Johnston | 5inch単装砲 Mk.30改 + 5inch単装砲 Mk.30改
                    expect(
                        check.aaci(562, [313, 313]).map(obj => obj.id)
                    ).toEqual([37]);
                    // Johnston | 5inch単装砲 Mk.30改+GFCS Mk.37 + 5inch単装砲 Mk.30改
                    expect(
                        check.aaci(562, [308, 313]).map(obj => obj.id)
                    ).toEqual([35]);
                    // Johnston | 5inch単装砲 Mk.30改+GFCS Mk.37 + 5inch単装砲 Mk.30
                    expect(
                        check.aaci(562, [308, 284]).map(obj => obj.id)
                    ).toEqual([35]);

                    // Johnston改 | 5inch単装砲 Mk.30改+GFCS Mk.37 + 5inch単装砲 Mk.30改+GFCS Mk.37 + 5inch単装砲 Mk.30改
                    expect(
                        check.aaci(689, [308, 308, 313]).map(obj => obj.id)
                    ).toEqual([34, 35]);
                    // Johnston改 | 5inch単装砲 Mk.30改 + 5inch単装砲 Mk.30改 + GFCS Mk.37
                    expect(
                        check.aaci(689, [313, 313, 307]).map(obj => obj.id)
                    ).toEqual([36, 37]);
                    // Johnston改 | 5inch単装砲 Mk.30改+GFCS Mk.37 + 5inch単装砲 Mk.30改 + 5inch単装砲 Mk.30改
                    expect(
                        check.aaci(689, [308, 313, 313]).map(obj => obj.id)
                    ).toEqual([35, 37]);
                    // Johnston改 | 5inch単装砲 Mk.30改+GFCS Mk.37 + 5inch単装砲 Mk.30改+GFCS Mk.37 + GFCS Mk.37
                    expect(
                        check.aaci(689, [308, 308, 307]).map(obj => obj.id)
                    ).toEqual([34, 5, 8]);
                });
                it(`Atlanta改`, function() {
                    // 5inch連装両用砲(集中配備) + GFCS Mk.37＋5inch連装両用砲(集中配備)
                    expect(
                        check.aaci(696, [362, 363]).map(obj => obj.id)
                    ).toEqual([39]);
                    // 5inch連装両用砲(集中配備) + GFCS Mk.37＋5inch連装両用砲(集中配備) + GFCS Mk.37
                    expect(
                        check.aaci(696, [362, 363, 307]).map(obj => obj.id)
                    ).toEqual([40, 39, 5, 8]);
                    // 5inch連装両用砲(集中配備) + 5inch連装両用砲(集中配備)
                    expect(
                        check.aaci(696, [362, 362]).map(obj => obj.id)
                    ).toEqual([41]);
                    // 5inch連装両用砲(集中配備) + 5inch連装両用砲(集中配備) + GFCS Mk.37
                    expect(
                        check.aaci(696, [362, 362, 307]).map(obj => obj.id)
                    ).toEqual([40, 41, 5, 8]);
                });
                it(`Other samples`, function() {
                    expect(
                        check.aaci(428, [130, 130, 124]).map(obj => obj.id)
                    ).toEqual([5, 8]);
                    expect(
                        check.aaci(330, [122, 122, 106]).map(obj => obj.id)
                    ).toEqual([1, 2, 3]);
                });
            });
        });
